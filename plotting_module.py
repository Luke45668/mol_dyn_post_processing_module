...
def thermo_variables_plot_against_strain_show_all_reals(strain_total,
...
def thermo_variables_plot_against_strain_show_all_reals_gpt(
    ...
    for j in range(indep_var_1.size):
        for i in range(indep_var_2_size[j]):
            fig, axes = plt.subplots(1, 4, figsize=(fig_width, fig_height))  # 1 row, 4 columns
            fig.suptitle("$\dot{\gamma}="+str(indep_var_2[i])+", K="+str(indep_var_1[j])+",N_{c}=$"+str(tchain[j]))
            

            for k in range(realisation_count):
                for ax, (label, (col_idx, low_lim, up_lim, lim_switch, trim)) in zip(axes, energy_vars.items()):
...
def stress_tensor_strain_time_series( n_outputs_per_stress_file,
                                     strain_total,
                                     fontsize_plot,
                                     indep_var_1,
                                     indep_var_2,
    ...
    for j in range(indep_var_1.size):
        for i in range(indep_var_2_size[j]):
            fig, axes = plt.subplots(1, len(stress_vars), figsize=(fig_width, fig_height))  # 1 row, n columns
            fig.suptitle("$\dot{\gamma}="+str(indep_var_2[i])+", K="+str(indep_var_1[j])+",N_{c}=$"+str(tchain[j]))
            ...
            for k in range(realisation_count):
                for ax, (label, (tensor_idx)) in zip(axes, stress_vars.items()):
                    ...
                    grad_cutoff=int(np.round(ss_cut*mean_stress_tensor[k,:,tensor_idx].size))
                    SS_grad=np.mean(np.gradient(mean_stress_tensor[k,grad_cutoff:,tensor_idx],axis=0))
