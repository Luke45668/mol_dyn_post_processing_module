...
for l in range(3):
    plt.scatter(erate[:e_end[0]],ordered_eigenvalue_matrix[l], label=value_index[l], color=colour[l])
plt.yscale('log')
...
for k in range(3): # loop through eigenvalues
    ...
    for l in range(3): # loop through components of eigenvectors
        plt.scatter(erate[:e_end[0]],ordered_eigenvectors_array[:,k,l], label=compontent_index[l], marker=marker[l], color=colour[k])
