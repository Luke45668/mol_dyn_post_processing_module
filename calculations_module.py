...
def stress_tensor_averaging_batch(e_end, labels_stress, trunc1, trunc2, spring_force_positon_tensor_batch_tuple, j_):
    ...
    trunc1_cutoff = int(np.round(trunc1 * spring_force_positon_tensor_batch_tuple[0][0].shape[0]))
    trunc2_aftercutoff = int(np.round(trunc2 * spring_force_positon_tensor_batch_tuple[0][0].shape[0]))
    ...
    for l in range(6):
        for i in range(e_end):
            for j in range(j_):
                ...
                data = spring_force_positon_tensor_batch_tuple[i][j][ cutoff:aftercutoff, :, l].ravel()
